...
if __name__ == "__main__":
    ...
    current_mem = torch.mps.current_allocated_memory() / (1024 ** 3)  # Convert to GB
    # Get total allocated memory by driver in GB
    driver_mem = torch.mps.driver_allocated_memory() / (1024 ** 3)  # Convert to GB

    print(f"Current MPS memory allocated: {current_mem:.2f} GB")
    print(f"Driver MPS memory allocated: {driver_mem:.2f} GB")
